...
import SEVIC.ChiTietSanPhamService;
import SEVIC.SanPhamService;
import SEVIC.ThuocTinhService;
...
//        addCboLoc(ctspsv.getTenChatLieuALL(), cboChatLieu1);
    ...
    private void cboLoaiThuocTinh() {
        ...
        listLoaiThuocTinh.add("Kích Thước");
    ...
    private void fillToTableKichCo(List<KichThuoc> list) {
        DefaultTableModel dtm = (DefaultTableModel) tblThuocTinh.getModel();
        dtm.setRowCount(0);
        int i = 1;
        for (KichThuoc kc : list) {
            dtm.addRow(new Object[]{
                i++,
                kc.getMaKichThuoc(),
                kc.getTen(),
    ...
    private KichThuoc readFormKichCo() {
        String ma = txtMaThuocTinh.getText();
        String ten = txtTenThuocTinh.getText();
    ...
    private ChiTietSanPham readCTSP() {
        ...
        String id_KichThuoc = String.valueOf(ctspsv.getIDKichThuoc(cboKichCo.getSelectedItem().toString()));
        ...
        String maVach = null;
        ChiTietSanPham ctsp = new ChiTietSanPham(maCTSP, id_SanPham, giaBan, soLuongCon, id_MauSac, id_KichThuoc, id_Hang, id_ChatLieu, trangThaiXoa, mota);
...
//                        fillToTableChatLieu(tts.getAllChatLieu());
            ...
            case "Hãng":
                ...
                break;
            case "Kích thước":
                int check3 = JOptionPane.showConfirmDialog(this, "bạn muốn thêm kích thước không");
                String ma2 = txtMaThuocTinh.getText().trim();
                String ten2 = txtTenThuocTinh.getText().trim();
                if (!kiemTraChuoi("^[a-zA-Z0-9 ]{1,19}$", ma2)) {
                    JOptionPane.showMessageDialog(this, "mã kích thước là chữ ko dấu ít hơn 20 kí tự");
                    return;
                }
                if (!kiemTraChuoi("^[a-zA-ZÀ-ỹ ]{1,29}$", ten2)) {
                    JOptionPane.showMessageDialog(this, "tên kích Thước là chữ ít hơn 30 kí tự");
                    return;
                }
                ...
                if (check3 == JOptionPane.YES_OPTION) {
                    if (tts.addKichThuoc(readFormKichCo()) > 0) {
                        JOptionPane.showMessageDialog(this, "Thêm Kích thước Thành Công");
                        fillToTableKichCo(tts.getAllKichThuoc());
                        addCbo(ctspsv.getTenKichCo(), cboKichCo);
                        moiThuocTinh();
                    } else {
...
//                        fillToTableChatLieu(tts.getAllChatLieu());
            ...
            case "Hãng":
                ...
                break;
            case "Kích Thước":
                int check3 = JOptionPane.showConfirmDialog(this, "bạn muốn sửa kích Thước không");
                String ma3 = txtMaThuocTinh.getText().trim();
                String ten3 = txtTenThuocTinh.getText().trim();
                if (!kiemTraChuoi("^[a-zA-Z0-9 ]{1,19}$", ma3)) {
                    JOptionPane.showMessageDialog(this, "mã kích Thước là chữ ko dấu ít hơn 20 kí tự");
...
//    }
    ...
    private void initComponents() {
        ...
        tblCTSP.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "STT", "Mã CTSP", "Tên Sản Phẩm", "Đơn Giá", "Số Lượng", "Màu Sắc", "Kích Thước", "Hãng", "Chất Liệu", "Ngày Tạo", "Ngày Sửa Cuối", "Trạng Thái", "Mô Tả"
            }
        ));
